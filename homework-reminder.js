...
//   2. todo.Fach ∈ morgens Fächer         (Fach-Abgleich, falls Feld in Notion existiert)
//
// Voraussetzung für Kriterium 2:
//   In der Schulaufgaben-Notion-DB ein Select-Feld "Fach" anlegen mit denselben
//   Werten wie im Stundenplan (Englisch, Mathematik, Deutsch, …).
//   Ohne das Feld funktioniert der Reminder trotzdem – nur via Fällig-Datum.
...
async function getTodosGefiltert(morgenDatum, faecherMorgen) {
  ...
  for (const page of rows) {
    ...
    // Fach ist ein optionales Select-Feld in der Schulaufgaben-DB
    const fach        = p['Fach']?.select?.name || '';

    const istFaelligHeute    = faellig && faellig < morgenDatum;  // überfällig
    const istFaelligMorgen   = faellig === morgenDatum;
    const istFachRelevant    = fach && faecherMorgen.has(fach);
