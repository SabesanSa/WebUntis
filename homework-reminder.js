...
function getReq(hostname, path, extraHeaders = {}) {
...
async function notionPage(pageId) {
  const res = await getReq('api.notion.com', `/v1/pages/${pageId}`, {
    'Authorization': `Bearer ${NOTION_TOKEN}`, 'Notion-Version': '2022-06-28'
  });
  return res;
}
...
// ── Fachname aus Relation-Page holen (gecacht) ────────────────────────────────

const fachCache = {};

async function resolveFachName(pageId) {
  if (fachCache[pageId]) return fachCache[pageId];
  try {
    const page = await notionPage(pageId);
    // Fächer-DB hat "Fach" als Title-Property
    const name = page.properties?.['Fach']?.title?.[0]?.plain_text
               || page.properties?.['Name']?.title?.[0]?.plain_text
               || '';
    fachCache[pageId] = name;
    return name;
  } catch {
    return '';
  }
...
async function getTodosGefiltert(morgenDatum, faecherMorgen) {
  ...
  // Fach-Relationen aller Todos parallel auflösen
  const todos = await Promise.all(rows.map(async page => {
    ...
    const fachNamen = await Promise.all(
      fachRelationen.map(r => resolveFachName(r.id))
    );
    const fach = fachNamen.filter(Boolean)[0] || '';

    return { title, faellig, prioritaet, fach, fachNamen };
  }));
  ...
  for (const todo of todos) {
    if (!todo) continue;
    const { title, faellig, prioritaet, fach, fachNamen } = todo;

    const istFaelligHeute  = faellig && faellig < morgenDatum;
...
async function main() {
  ...
  const [faecherMorgen, stunden] = await Promise.all([
  ...
  const { relevant, anzahlAndere } = await getTodosGefiltert(morgenDatum, faecherMorgen);
  ...
  if (relevant.length === 0) {
    ...
    for (const t of relevant) {
      const icon = pEmoji[t.prioritaet] || '•';
      let grund = '';
  ...
  console.log(`✅ Reminder gesendet (${relevant.length} relevant, ${anzahlAndere} andere)`);
}
