...
async function fetchTimetable() {
  ...
  try {
    ...
    if (zulassen) { await zulassen.click(); await page.waitForTimeout(3000); }
    console.log("Eingeloggt: " + page.url());

    // Alle 3 Wochen parallel in separaten Tabs laden
    await Promise.all([0, 1, 2].map(async (offset) => {
      const dateStr = toISODate(getMondayOfWeek(offset));
      console.log("Starte Woche: " + dateStr);
      ...
      tab.on("response", async (response) => {
        ...
        try {
          ...
          const lessons = extractLessons(json);
          if (lessons.length > 0) {
            console.log("Woche " + dateStr + ": " + lessons.length + " Eintraege");
            capturedLessons.push(...lessons);
          }
        } catch {}
      });

      const responsePromise = tab.waitForResponse(
        r => r.url().includes("timetable/entries") && !r.url().includes("settings"),
        { timeout: 15000 }
      ).catch(() => null);
      await tab.goto("https://wkdo.webuntis.com/timetable/my-student?date=" + dateStr, { waitUntil: "domcontentloaded", timeout: 20000 });
