...
async function fetchTimetable() {
  ...
  page.on("response", async (response) => {
    ...
    try {
      ...
      if (lessons.length > 0) {
        console.log("🎯 " + lessons.length + " Stunden abgefangen!");
  ...
  try {
    ...
    if (zulassen) { console.log("✅ Zulassen..."); await zulassen.click(); await page.waitForTimeout(4000); }
...
async function sync() {
  ...
  if (allLessons.length === 0) { console.warn("⚠️ Keine Stunden."); process.exit(1); }
  ...
  for (const date of dates) {
    ...
    const del = await deleteEntriesForDate(date);
    for (const l of d) await createEntry(l);
    console.log("  📆 " + date + ": " + (del > 0 ? del + " alt, " : "") + d.length + " neu");
  }
  console.log("\n✨ Fertig!");
}
